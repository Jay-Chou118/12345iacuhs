...
from can.io.generic import *
from can.io.generic import FileIOMessageWriter, MessageReader
...
class myBLFReader(BaseIOHandler):
    ...
    def __init__(self, file):
        ...
        """
        super(myBLFReader, self).__init__(file, mode='rb')
            ...
            while True:
                ...
                if obj_type == LOG_CONTAINER:
                    ...
                    while pos + OBJ_HEADER_BASE_STRUCT.size < len(data):
                        ...
                        while header[0] != b"LOBJ" and len(data) > 3:
                            # print('��ѭ��')
                            data = data[1:]
                            header = OBJ_HEADER_BASE_STRUCT.unpack_from(data, pos)
                        # if header[0] != b"LOBJ":
...
class myBLFReader2(MessageReader):
    """
...
class myBLFWriter(FileIOMessageWriter):
    """
