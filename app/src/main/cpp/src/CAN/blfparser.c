...
/* in-memory zlib compress */
static success_t blfMemCompress(uint8_t  *next_out,
     uint32_t  avail_out,
     uint8_t  *next_in,
     uint32_t  avail_in,
     uint32_t *total_out_ptr, int level)
{
  int zres;
  z_stream stream;
  ...
  zres = deflateInit_(&stream, level, ZLIB_VERSION, sizeof(stream));
  if(zres == Z_OK) zres = deflate(&stream, Z_FINISH);
  if(zres == Z_STREAM_END) zres = Z_OK;
  if(zres == Z_OK) {
    deflateEnd(&stream);
    if(total_out_ptr != NULL) {
...
static void
blfSystemTimeInit(SYSTEMTIME_ *const s)
{
  s->wYear = 0;
  s->wMonth = 0;
  s->wDayOfWeek = 0;
        ...
        uint32_t totalBytesToRead)
{
  ...
  while(alreadyRead < totalBytesToRead) {
    ...
    if(srcBytesLeft) {
      ...
      if(!blfDualStreamReadOrSkip(ds, destPtr, wantToRead, 0)) 
	  	goto fail;
      ...
      if(!blfDualStreamReduceBytesLeft(ds, actuallyRead)) 
	  	goto fail;
	...
	{
      if(!blfSizedStreamDetermineBytesLeft(&ds->fileStream)) 
	  	goto fail;
    }
  }
  return 1;
...
success_t blfCompress(VBLObjectHeaderBaseLOGG* hbaselogg,
      uint8_t **bufPtr, uint32_t *sizePtr, int level)
{
  if(hbaselogg->compressedflag == 2) {
    uint8_t* uncompressedData;
    success_t suc;
    uint32_t inflatebuffersize = 0;
    ...
    suc = blfMemCompress(uncompressedData,
         hbaselogg->deflatebuffersize, *bufPtr,
         *sizePtr, &inflatebuffersize, level);

    /* free compressed data */
    //free(*bufPtr);

    hbaselogg->base.mObjectSize = inflatebuffersize + sizeof(VBLObjectHeaderBaseLOGG);
	memcpy(*bufPtr, uncompressedData, inflatebuffersize);
    free(uncompressedData);
    if(!suc) {
      fprintf(stderr, "blfCompress failed\n");
      goto fail;
    }
  } else {
    /* nothing to do. sizePtr and bufPtr point to the non-compressed data */
...
blfHandleReadLOGG(VBLObjectHeaderBaseLOGG *hbaselogg, BLFHANDLE h)
{
  ...
  if(hbaselogg->base.mObjectType != BL_OBJ_TYPE_LOG_CONTAINER) 
  	goto success;

  /* fail, if memStream is not drained yet */
  if(blfSizedStreamIsOpen(&ds->memStream))                     
  	goto fail;

  /* complete reading of VBLHeaderBaseLOGG */
  if(!blfHandleRead(h, 0, (uint8_t *)&hbaselogg->compressedflag,
        sizeof(*hbaselogg) - hbaselogg->base.mHeaderSize))     
        goto fail;

  /* allocate object data */
  nObjectData = hbaselogg->base.mObjectSize - sizeof(*hbaselogg);
  if(NULL == (objectData = (uint8_t *)malloc(nObjectData)))    
  	goto fail;

  /* read object data */
  if(!blfHandleRead(h, 0, objectData, nObjectData))            
  	goto fail;
  if(!blfHandleSkipPadding(h, nObjectData))                    
  	goto fail;

  /* uncompress, if compressed */
  if(!blfLOGGUncompress(hbaselogg, &objectData, &nObjectData)) 
  	goto fail;

  printf("CONTAINER:objSize:%d comSize:%ld UncomSize:%d pad:%ld\r\n", 
  		hbaselogg->base.mObjectSize, hbaselogg->base.mObjectSize - sizeof(*hbaselogg), 
  		nObjectData, (hbaselogg->base.mObjectSize - sizeof(*hbaselogg)) & 3);
#if 0
	 static int i = 0;
  	if (i < 10){
		char name[32] = "test_uncompress.blf%d";
		sprintf(name, "test_uncompress%d.blf", i);
		FILE *fp_uncom = fopen(name, "wb");
  		fwrite(objectData, 1, nObjectData, fp_uncom);
		fclose(fp_uncom);
		i++;
  	}
#endif	
  ...
  if(   (h->mPeekFlag == 0)
     && (!blfPeekObject(h, &(hbaselogg->base))))              
     goto fail;
